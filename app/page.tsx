 ...
 * app/page.tsx
 *
 * Root server page. Reads lat/lon/city from URL search params (default: Nairobi).
 * Fetches current weather and hourly forecast server-side via the active
 * IWeatherProvider, then passes data down to display components.
 * WeatherShell handles client-side search.
 */

import { weatherProvider } from "@/app/lib/providers";
import { DEFAULT_LAT, DEFAULT_LON, DEFAULT_CITY } from "@/app/lib/config";
...
export default async function HomePage({ searchParams }: PageProps) {
  ...
  const [conditions, hourlySlots] = await Promise.all([
    weatherProvider.getCurrentWeather(lat, lon, city),
    weatherProvider.getHourlyForecast(lat, lon),
  ]);
